...
public class OddEvenJump {
    ...
    /** completed borrowed from lee215 who said his solution was suggested by zhangchunlei0813
     *
    ...
    public int oddEvenJumps_dp(int[] A) {
        ...
        tm.put(A[N-1], N-1);
        for(int i = N - 2; i >= 0; --i) {
            Integer ceiling = tm.ceilingKey(A[i]);
            if(ceiling != null) dp[i][1] |= dp[tm.get(ceiling)][0];
            Integer floor = tm.floorKey(A[i]);
            if(floor != null) dp[i][0] |= dp[tm.get(floor)][1];
            tm.put(A[i], i);
        }
