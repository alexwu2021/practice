...
public class VisiblePoints {
    ...
    public int visiblePoints(List<List<Integer>> points, int angle, List<Integer> location) {
        ...
        for (List<Integer> p : points) {
            ...
            ang.add(Math.atan2(dy, dx) * (180 / Math.PI));
        }
        Collections.sort(ang);

        List<Double> ang2 = new ArrayList<>(ang);
        for(double a : ang) ang2.add(a + 360);

        int res = count;
        for (int i = 0, j = 0; i < ang2.size(); ++i) {
            while (ang2.get(i) - ang2.get(j) > angle) {
                ++j;
            }
